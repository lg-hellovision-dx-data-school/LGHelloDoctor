...
function sttUploadFilename(blob: Blob): string {
  const t = (blob.type || '').toLowerCase()
  if (t.includes('webm')) return 'wake-word.webm'
  if (t.includes('wav')) return 'wake-word.wav'
  if (t.includes('ogg')) return 'wake-word.ogg'
  if (t.includes('mp4') || t.includes('mp4a') || t.includes('aac')) return 'wake-word.m4a'
  return 'wake-word.webm'
}
...
export async function postSttAudio(audioBlob: Blob): Promise<string> {
  ...
  form.append('audio', audioBlob, sttUploadFilename(audioBlob))
