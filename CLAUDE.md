...
| HITL 체크포인트 | 위치 | 사람의 역할 |
|----------------|------|-----------|
| **위험 명령어 차단** | `.claude/settings.json` 훅 | AI가 `rm -rf`, `git push --force` 등 실행 시 자동 차단 → 사람이 명시적으로 허용 여부 결정 |
| **배포 전 통합 검증** | `/validate` 슬래시 명령어 | 사람이 직접 실행하고 7개 항목 체크리스트 결과를 눈으로 확인 후 배포 승인 |
| **테스트 결과 검토** | `/test` 슬래시 명령어 | pytest 결과를 사람이 리뷰한 뒤 머지 또는 수정 결정 |
| **응급 키워드 큐레이션** | `EMERGENCY_KEYWORDS` | AI가 제안한 키워드를 사람이 검토·추가·삭제하여 최종 승인 |
| **금지어 목록 관리** | `FORBIDDEN_WORDS` | 의료법 준수 여부를 사람이 판단하고 10개 항목 직접 확정 |
| **품질 임계값 설정** | 한국어 비율 40% 기준 | AI 출력 품질 기준을 사람이 실험 후 수동으로 설정 |
| **편집 후 알림** | `settings.json` PostToolUse 훅 | AI가 파일 수정 후 사람에게 알림 → 사람이 diff 확인 후 승인 |
